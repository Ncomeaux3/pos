...
function ToastItem({ message, onDone }: { message: string; onDone: () => void }) {
  ...
  return (
    <div className="glass-panel pointer-events-auto rounded-full px-4 py-2.5 text-[13px] font-medium text-ink shadow-[var(--pop)] duration-200 ease-[var(--ease)] animate-in fade-in slide-in-from-bottom-2">
      {message}
    </div>
  )
}
