...
| 3c Motion and row swipes | Phone push, pop and tab cross-fade through React `ViewTransition`, the stagger kept on desktop; one `SwipeRow` on Review, Notifications and Tasks (snooze) | medium | 2 | 3b | PR open 2026-09-21: push and tab fade through React `ViewTransition`, pop through the browser API (a popstate transition is flushed synchronously and never animates in React), `SwipeRow` on the three screens; owner phone check pending | #115 |
