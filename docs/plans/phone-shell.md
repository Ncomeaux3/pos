...
Done 2026-09-13. One change from the text below: a bare `history.pushState` makes the router restore the page segment it last had for that URL, and when that write raced a server action (delete a task, then close its row) the restore showed the list from before the delete. `useSearchState` writes the URL at once with the router's own history state attached (an entry the router treats as its own and leaves alone), returns the new params from a local override so the sheet opens within a frame, and asks the router to replace the same URL so the fetch is queued in order with server actions. See decisions/log.md. Two e2e tests fail on main as well and are not this phase's: "home, a snooze comes back" and "settings notifications, push says what it needs".
