...
Done 2026-09-13. One change from the text below: a `history.pushState` makes the router restore the page segment it last had for that URL, and when that write raced a server action (delete a task, then close its row) the restore showed the list from before the delete, about once in thirty runs under a slow network even with the router's own state attached. `useSearchState` asks the router to push or replace instead (queued in order with server actions) and returns the new params from a shared override, so the sheet opens within a frame and the URL follows one fetch behind. A test that reloads after opening something waits for the URL first. See decisions/log.md. Two e2e tests fail on main as well and are not this phase's: "home, a snooze comes back" and "settings notifications, push says what it needs".
