...
|---|---|---|---|---|---|---|
| 1 Diagnose and small fixes | Push and email diagnosed with evidence; snooze, deep links, health corner, time clear, splash, nightly email fixed | medium | 3, 8, 9, 10 | none | Not started | |
...
- [ ] Task time clear: in `TaskDrawer.tsx` next to the `type="time"` input (line 204) a small "clear" button when `draft.time` is set that sets it to `''`. Save already sends null for empty.
