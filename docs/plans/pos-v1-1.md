...
- [x] Layout setting: `dashboard_layout: { order: string[]; hidden: string[] } | null` in `Settings` and `DEFAULT_SETTINGS` (`core.settings` is key/value jsonb, no migration). Server action `saveDashboardLayout` in `shell-actions.ts`: `requireOwner`, zod `array(string().regex(/^[a-z0-9_-]{1,40}$/)).max(40)` for both lists, `setSetting`. The key is fixed in the action.
- [x] `Bento.tsx`: take `layout` as a prop from `page.tsx`, delete the localStorage store and `useSyncExternalStore`; arrange mode adds Hide/Show beside the arrows and a hidden list at the foot; Reset saves null. One layout for both widths; `PHONE_TILES` stays the phone subset.
- [x] Shrink: `[grid-auto-rows:minmax(200px,auto)]` to `auto` and drop `h-full` from `tileClass` so a tile is as tall as its content; ui-verifier judges `content-start`.
- [x] Notification href: migration adds `href text` to `core.notifications` (existing core table: a column add restates nothing). `queue()` takes `href?`; insurance `nightly-digest.ts:29` passes `/insurance`; `assembleSummary` passes `/notifications`. `WarningList` row title becomes a Link to `href ?? '/notifications'`.
- [x] Freshness, write side: in `core/tools.ts` after `const result = await tool.run(...)` (line 139) and before the write log: `if (toolName !== 'get_digest') await writeDigest(manifest)` in a try/catch that logs and never fails the write. `query` returned at line 98; `get_digest` is the one read a manifest names; everything else is a write by contract. `writeDigest(manifest)` is the loop body of `writeDigests()` extracted in `core/digests.ts`.
- [x] Freshness, read side: `page.tsx` module tiles map over `await latestDigests()` (`core/digests.ts:39`) filtered to enabled modules, instead of `summary.modules`. `latestSummary()` still supplies the headline, alerts and Last run. The Finance tile therefore appears whenever finance has any digest, which is the "finance graph missing" fix.
- [x] Prune: `core.digests` grows one row per write. Nightly `prune` keeps every row from the last 2 days and past that only the newest per module per day, so `digestsBefore(7)` still finds last week.
- [x] Desktop reload and pull-to-refresh already re-render (every action calls `revalidatePath('/', 'layout')`; `PullToRefresh` calls `router.refresh`); assert, do not change.
- [x] e2e: complete a task on `/tasks`, go to `/`, the Tasks tile moved without Run now; hide a tile, reload, still hidden; a warning row is a link.
...
Done 2026-09-15. `writeDigest` measured on the local database, warm: finance 3 ms, skills 4 ms, every other module 1 ms (cold first call 34 ms for finance); one more query round trip per write on production. The migration is `20260915150100_core_notification_href.sql` (`150000` clashed with Phase 6's `tasks_project_goal`, written in the same hour; Phase 6 moved to `160000`). Phases 5 and 6 were built in parallel worktrees (`../pos-phase-5`) after both sessions landed in one checkout; a phase session should start in its own worktree. Hidden tiles are dropped from the phone too, since there is one layout. Shrink: rows are `auto` but `h-full` stays on the card; the ui-verifier measured the wrapper cell still stretching to the row's tallest tile, so dropping `h-full` gave ragged bottoms (Tasks 225 in a 362 cell) and a 102px empty state, and the three rows under 200px are the whole saving (80px at 1440 against today's data). `approve()` in `core/proposals.ts` recomputes the digest too, since it runs `tool.run` without `callTool`. Insurance's `get_digest` no longer queues reminders: the per-write recompute would have queued one per policy edit, so queueing moved to the `remind` job (which also stops the nightly's double queue, since it ran `nightlyDigest` as the job and again in `writeDigests`). CI's screens job then failed on the fitness screen: its goal tile read the goals digest, which the earlier check-in test now updates at once, and `fitnessGoal` in `modules/fitness/data.ts` matched a tasks goal through the shared `unclassified` skill id, a pre-existing bug the stale digest had hidden; `unclassified` is now excluded and the test accepts the goal at 340 stalled or 355 at risk. Not done, pre-existing, from the ui-verifier: the Next 7 days tick labels are 1.78:1 and the first marker overdraws "Today"; ink-3 labels at 10px are under 4.5:1; the arrange bar differs from the artboard (separate strip, ink-2 arrows); `/?arrange=1` typed on the phone renders arrange mode; the Hide button is 17px tall. `e2e/screens.spec.ts` snooze test now reads the warning title from `a.text-ink`, the row title being a link.
...
- [ ] Owner step (OWNER-TODO 15): Health Auto Export Premium, one export to the webhook. Connected 2026-09-18; the fixture from the share sheet is still to come. Note for when it is: `core.request_log` stores route, status and error only, no body, so the fixture has to come from the app's own share sheet or a request bin, not from the Test view. Save it as the fixture for `client.test.ts`; correct the names marked verify in `client.ts`.
