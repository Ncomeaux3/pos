...
| Mailing address incomplete | **Owner action, blocks merge** | `core/legal.ts` `address` has `[city]` and `[ZIP]` placeholders shown on all three pages. A PO box or virtual mailbox keeps a home address private. |
