...
| Apple feeds: subscribed calendars and Reminders (v1.2 phase 7b) | Clear | Both are in the Privacy Policy's "what comes from services you connect" list, with the warning that a published calendar URL is readable by anyone who has it. No Apple API and no account: a calendar is fetched from its own URL, reminders are posted by a Shortcut on the owner's phone. Neither is sent to an AI provider, though a reminder that becomes a task is classified to skills like any task, which the policy says. |
