...
Read 2026-09-23 and 2026-09-24 through the JSON behind each page (`developer.apple.com/tutorials/data/design/human-interface-guidelines/<slug>.json`). Every section names the pages it rests on. A number marked "verify" was taken from notes or a secondary source and has not been checked against Apple's page.
...
- iOS default body is 17pt; minimum text size is 11pt.
...
- Text scales up to the AX5 accessibility size. macOS Body is 13pt (verify leading 16).
...
- 12 system colours, each with light, dark, accessible-light and accessible-dark values. Blue, for example: 0,136,255 / 0,145,255 / 30,110,244 / 92,184,255 (verify, from notes).
- System grey 1 to 6.
...
- **Offering help** (hig/offering-help): contextual tips of one or two sentences; tooltips start with a verb and stay short (60 to 75 characters, verify).
...
- **Buttons** (hig/buttons): 44x44pt targets; roles are normal, primary, cancel and destructive; a destructive action is never the primary button; one or two prominent buttons per view.
- **Menus** (hig/menus): title case, verb first, an ellipsis when the item needs more input, one level of submenu.
- **Segmented controls** (hig/segmented-controls): five segments or fewer on iPhone; text or icons, never mixed; they select state, never perform an action.
...
- **Popovers** (hig/popovers): one at a time.
- **Alerts** (hig/alerts): a title; at most three buttons; no Yes or No labels; Cancel is never the default.
- **Tab bars** (hig/tab-bars): five tabs or fewer on iPhone; floating glass; minimises on scroll.
...
- **Sidebars** (hig/sidebars): at most two levels; not hidden by default.
...
- **UI kits.** Apple Design Resources publishes Figma and Sketch kits for iOS, iPadOS, macOS, tvOS, watchOS and visionOS. Their reuse terms were not read (verify before copying anything from a kit).
