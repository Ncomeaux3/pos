...
3. Nightly cron, in order: refresh OAuth tokens, each module's jobs, embed changed entities, each module's `get_digest` into `core.digests`, orchestrator assembles `core.dashboard_summary` by rules and asks Haiku for a two sentence headline, queues one notification, prunes `request_log`, thins `core.digests` to one row per module per day past two days.
